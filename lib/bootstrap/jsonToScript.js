...
const createContentType = (item, editorInterface) => `
  const contentType = migration.createContentType('${item.id}')${''.concat(...Object.entries(item.props).map(createProp))};
  ${''.concat(...item.fields.map(field => createField('contentType', field)))}
  ${editorInterface.map(field => createChangeFieldControl('contentType', field)).join('\n')}
`
...
const jsonToScript = (contentTypeJson, editorInterface) => {
  ...
  console.log(engine.executeOnText(unformattedScript));
  console.log(engine.executeOnText(unformattedScript).results[0].messages);
